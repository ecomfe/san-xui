...
UUID.prototype._init = function () {
    let names = UUID.FIELD_NAMES,
        sizes = UUID.FIELD_SIZES;
    let bin = UUID._binAligner,
        hex = UUID._hexAligner;
   ...
   */
    ...
    for (let i = 0; i < 6; i++) {
        let intValue = parseInt(arguments[i] || 0, 10);
...
UUID.genV1 = function () {
    ...
    // format time fields
    let tf = UUID._getTimeFieldValues(st.timestamp);
    let tl = tf.low + st.tick;
    let thav = (tf.hi & 0xFFF) | 0x1000; // set version '0001'

    // format clock sequence
    st.sequence &= 0x3FFF;
    let cshar = (st.sequence >>> 8) | 0x80; // set variant '10'
    let csl = st.sequence & 0xFF;
